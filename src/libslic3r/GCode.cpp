...
std::string GCodeGenerator::extrude_loop(const ExtrusionLoop &original_loop, const std::string_view description, double speed)
{
    ...
    if (scarf_seam_perimeter && !m_layer_collision_already_printed_2_width.empty() && m_layer && !paths.empty() && !paths.front().empty()) {
        ...
        {
            BoundingBox bbox = get_extents(forbidden);
            BoundingBox bbox2 = get_extents(pl.points);
            bbox.merge(bbox2);
            bbox.offset(scale_(1.));
            static int iRun=0;
            ::Slic3r::SVG svg(debug_out_path("%d_%d_%d_scarf_check.svg", m_layer->id(), iRun++).c_str(), bbox);
            svg.draw(forbidden, "blue");
            for (const std::pair<ArcPolylines, coord_t> &arcpoly_2_width : m_layer_collision_already_printed_2_width) {
                svg.draw(to_polylines(arcpoly_2_width.first, scale_(max_nozzle / 2)), "green", arcpoly_2_width.second / 2);
            }
            svg.draw(pl, "red", scale_(0.05));
            svg.draw(to_polylines(forbidden), "cyan", scale_(0.04));
            svg.Close();
        }
        assert(pl.size() > 0);
